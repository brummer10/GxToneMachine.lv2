...
G_DEFINE_TYPE(GxFoxxPaintBox, gx_paint_box, GTK_TYPE_BOX)

static void gx_paint_box_class_init (GxFoxxPaintBoxClass *klass)
{
...
static void set_paint_func(GxFoxxPaintBox *paint_box, const gchar *paint_func)
{
	gchar *spf;
...
static void gx_paint_box_style_set(GtkWidget *widget, GtkStyle  *previous_style)
{
	GxFoxxPaintBox *paint_box = GX_FOXX_PAINT_BOX(widget);
	set_paint_func(paint_box, paint_box->paint_func);
}


static void gx_paint_box_init (GxFoxxPaintBox *paint_box)
{
    ...
    paint_box->stock_image = gdk_pixbuf_new_from_resource("/gx_foxxtonemachine/pedal.png", NULL);
    //gdk_pixbuf_save(paint_box->stock_image,"plug.png","png",NULL);
}

static void gx_paint_box_destroy(GtkObject *object)
{
	GxFoxxPaintBox *paint_box = GX_FOXX_PAINT_BOX(object);
	...
	GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_PAINT_FUNC:
		g_value_set_string(value, GX_FOXX_PAINT_BOX(object)->paint_func);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
static void pedal_expose(GtkWidget *wi, GdkEventExpose *ev)
{
	...
	cairo_t *cr;
	GxFoxxPaintBox *paintbox = GX_FOXX_PAINT_BOX(wi);
