...
static void ref_value_changed(GtkAdjustment *adj, gpointer* args) {
	...
	gx_foxxtonemachineUI* ui = (gx_foxxtonemachineUI*)arg->ui;
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor*   descriptor,
    ...
    *widget = NULL;
    ...
    const char* plug_name = "GxToneMachine" ;
    ...
    set_expose_func(GX_FOXX_PAINT_BOX(ui->pbox),"pedal_expose");
    ui->box = gtk_vbox_new(FALSE, 4);
    ui->hbox = gtk_hbox_new(TRUE, 10);
    gtk_container_set_border_width(GTK_CONTAINER(ui->hbox),30);
    ...
    ui->label[1] = gtk_label_new("SUSTAIN");
    ...
    ui->args[1]->ui = ui;
    ui->args[1]->port_index = (int)SUSTAIN;
    g_signal_connect(G_OBJECT(ui->adj[1]), "value-changed",
          G_CALLBACK(ref_value_changed),(gpointer*)ui->args[1]);

    ui->adj[0] = gtk_adjustment_new( 0.0, 0.0, 1.0, 1.0, 1.0*10.0, 0);
    ui->knob[0] = gtk_switch_new_with_adjustment(GTK_ADJUSTMENT(ui->adj[0]));
    ui->label[0] = gtk_label_new("OCTAVE");
    ...
    ui->args[0]->ui = ui;
    ui->args[0]->port_index = (int)OCTAVE;
