...
void gx_gui_create_window_and_surface(gx_tonemachineUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_tonemachineUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_tonemachineUI *ui) {
	...
	UnregisterClass(TEXT("gx_tonemachineUIClass"), NULL);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_tonemachineUI *ui = (gx_tonemachineUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
